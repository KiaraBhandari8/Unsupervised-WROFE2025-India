...
def calculate_steering_angle_area(filtered_path_mask, max_angle=50):
    ...
    # print(f"Area Ratio : {area_diff_ratio}") # Commented out for cleaner logs
    steering_angle = area_diff_ratio * max_angle
    return steering_angle

# Removed final_steering_angle parameter from here
def get_robot_direction_and_angle(frame):
    ...
    Returns the direction, steering angle, and a visualized frame with basic annotations.
    ...
    if not contours_black_boundaries:
        # print("No black boundaries found. Stopping.") # Commented out for cleaner logs
        # Make sure visualized_frame is original frame for consistent output
    ...
    if relevant_path_contour is not None:
        ...
        # print("No significant path component found in the bottom 25% of the frame. Stopping.") # Commented out
        return "STOP", None, frame.copy()
    ...
    if M["m00"] != 0:
        ...
        # print(f"Reocommended Steering Angle from Centoriod : {steering_angle_centroid}") # Commented out
        # print(f"Reocommended Steering Angle from Area : {steering_angle_area}") # Commented out
        ...
        if np.sum(filtered_path_mask) < (w * h * 0.005):
            # print("Filtered path mask is mostly black (path likely lost). Stopping.") # Commented out
            command = "STOP"
            steering_angle = None
            cx_full = None
            cy_full = None
        ...
        # print("No significant path found in filtered mask. Stopping.") # Commented out
        command = "STOP"
        steering_angle = None
    ...
    if cx_full is not None and cy_full is not None:
        cv2.circle(visualized_frame, (cx_full, cy_full), 10, (0, 0, 255), -1) # Red circle for centroid (reduced size)
        cv2.line(visualized_frame, (w // 2, 0), (w // 2, h), (255, 0, 0), 2) # Blue center line (reduced thickness)
        ...
        if command in ["FORWARD", "LEFT", "RIGHT"] and steering_angle is not None:
            ...
            cv2.arrowedLine(visualized_frame, (arrow_start_x, arrow_start_y),
                            (arrow_end_x, arrow_end_y), (255, 255, 0), 3, tipLength=0.3) # Yellow arrow (reduced thickness)
    
    # Removed final_steering_angle drawing from here

    steering_angle = int(steering_angle) if steering_angle is not None else 0 # Ensure integer for return
...
if __name__ == "__main__":
    ...
    # For testing, provide a dummy final_steering_angle
    command, steering_angle, visualized_frame = get_robot_direction_and_angle(frame) # No angle passed here
